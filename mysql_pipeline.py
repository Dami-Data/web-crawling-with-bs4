...
# extract table schema from destination table. Because this is a simple pipeline, I already have the table on BQ with the schema matching what i have on AWS
def extract_schema(): 
    dataset_id = dataset
    table_id = table_name
...
def get_last_modified():
    q = """select max(created_at) last_date from {project_id}{dataset_id}{table_name}"""
...
# our main function
def main():
    send_notification('Processing {table_name} data ✅ at {}'.format(datetime.now()))
    ...
    if len(data)>0:
        print(len(data))
        send_notification('Found {} record of {table_name} data ✅✅ '.format(len(data)))
        # load the data to GBQ
        data.to_gbq({dataset_id}.{table_name}, project_id={project_id}, table_schema= extract_schema(), if_exists='append', chunksize=len(data)//4)
        send_notification('Successfully Added {table_name} data to BQ ✅✅✅ at {}'.format(datetime.now()))
    else:
        send_notification('Successfully, No new {table_name} data ✅✅ at {}'.format(datetime.now())) 
